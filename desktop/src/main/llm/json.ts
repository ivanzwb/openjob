...
import type { ComposedPrompt, PromptProvenance } from '@core/prompts/composer';
...
async function requestJson<T>(request: JsonRequest): Promise<T> {
  ...
  const fail = (message: string): Error => {
    recordPromptRun({ ...runRecord, ok: false, error: message, latencyMs: Date.now() - startedAt });
  ...
  if (reasoningRaw) {
    try {
      return succeed(parseJsonResponse<T>(reasoningRaw, parseOptions), reasoningRaw);
    } catch (err) {
      lastError = err;
    }
  }
