...
import type { CapabilityRegistry } from '@core/plugins/types';
...
// desktop/src/main/plugins/ → 仓库根。装载路径横跨 desktop 与 core 两个包，
// 扫描也必须横跨，否则包一拆，core 那半边就静静退出了扫描范围
...
describe('装载路径上没有代码执行入口', () => {
  ...
  it('每个接触包内容的模块都在扫描清单里', () => {
    ...
    const handlers = [CORE_SRC, DESKTOP_SRC].flatMap((srcRoot) =>
      readdirSync(srcRoot, { recursive: true, encoding: 'utf8' })
        .filter((entry) => entry.endsWith('.ts') && !entry.endsWith('.test.ts'))
        .map((entry) => join(srcRoot, entry))
        .filter((file) =>
          /from '[^']*plugins\/package\/(contract|replay)'/.test(readFileSync(file, 'utf8')),
        ),
    );
