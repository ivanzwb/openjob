...
// 按 __dirname 而不是 process.cwd() 定位手机端：cwd 取决于在哪个目录起的 vitest，
// 拆成 monorepo 之后那可能是仓库根，也可能是 desktop/
const REPO_ROOT = join(__dirname, '..', '..', '..', '..');
const MOBILE_MIGRATIONS_DIR = join(REPO_ROOT, 'mobile', 'src', 'db', 'migrations');
...
describe('plugin runtime persistence migration', () => {
  it('桌面与手机使用同一份 T03 DDL', () => {
    ...
    const mobile = readFileSync(
      join(REPO_ROOT, 'mobile', 'src', 'db', 'migrations', '0021_plugin_runtime_persistence.sql'),
      'utf8',
    ).replace(/\r\n/g, '\n');
...
describe('candidate evidence migration', () => {
  it('桌面与手机使用同一份 T10 DDL', () => {
    ...
    const mobile = readFileSync(
      join(REPO_ROOT, 'mobile', 'src', 'db', 'migrations', '0022_candidate_evidence.sql'),
      'utf8',
    ).replace(/\r\n/g, '\n');
    expect(mobile).toBe(desktop);
  });
...
describe('story migration', () => {
  it('桌面与手机使用同一份 T13 DDL', () => {
    ...
    const mobile = readFileSync(
      join(REPO_ROOT, 'mobile', 'src', 'db', 'migrations', '0024_story.sql'),
      'utf8',
    ).replace(/\r\n/g, '\n');
    expect(mobile).toBe(desktop);
  });
