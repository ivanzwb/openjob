...
describe('双端发布版本配置', () => {
  // 按 __dirname 而不是 process.cwd() 定位：cwd 取决于在哪个目录起的 vitest，
  // 拆成 monorepo 之后那可能是仓库根，也可能是某个子项目
  const REPO_ROOT = join(__dirname, '..', '..');
  const readJson = (...parts: string[]) =>
    JSON.parse(readFileSync(join(REPO_ROOT, ...parts), 'utf8')) as Record<string, never>;

  it('desktop package、mobile package 与 Expo app.json 保持精确一致', () => {
    // 版本号的唯一来源是 desktop/package.json（electron-builder 读它命名安装包）；
    // 工作区根的 package.json 不带 version，免得多一处会漂的事实源
    const desktop = readJson('desktop', 'package.json') as unknown as { version: string };
    const mobile = readJson('mobile', 'package.json') as unknown as { version: string };
    const appConfig = readJson('mobile', 'app.json') as unknown as {
      expo: { version: string; android: { versionCode: number } };
    };

    expect(mobile.version).toBe(desktop.version);
    expect(appConfig.expo.version).toBe(desktop.version);
    // versionCode 只取数值主干：预发布后缀（1.0.0-rc.1）没法编码进整数，
    // 而版本串本身必须带后缀一致，否则 rc 包会被当成正式包互相同步
    const trunk = desktop.version.split(/[-+]/)[0]!;
    expect(appConfig.expo.android.versionCode).toBe(
      Number(trunk.split('.').map((part) => part.padStart(2, '0')).join('')),
    );
  });

  it('工作区根不带 version，避免第二处版本事实源', () => {
    expect(readJson('package.json')).not.toHaveProperty('version');
  });
});
