 ...
 * IPC 契约回归防线。
 *
 * core/src/ipc.ts 的通道清单是契约的唯一事实源，但契约本身不会阻止：
...
// core/src/ → 仓库根。desktop 的源码是被扫描对象，不是依赖：这里只读文本，
// 不 import，所以 core 不会因为这条静态关卡反向依赖 desktop 包。
const REPO_ROOT = join(__dirname, '..', '..');
const DESKTOP_SRC = join(REPO_ROOT, 'desktop', 'src');

/** 递归收集 desktop/src/main 下所有 .ts 文件 */
...
const mainFiles = collectTsFiles(join(DESKTOP_SRC, 'main'));
...
describe('preload 白名单与契约一致', () => {
  it('preload 从契约构建放行名单，而不是硬编码', () => {
    const src = readFileSync(join(DESKTOP_SRC, 'preload', 'index.ts'), 'utf8');
