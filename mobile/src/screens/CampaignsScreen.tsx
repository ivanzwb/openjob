...
export function CampaignsScreen(): React.JSX.Element {
  ...
  const detail = detailId ? getCampaignDetail(getRawDb(), detailId) : null;
  ...
  const diagnose = async () => {
    ...
    setBusy(true);
    try {
      const { events } = await invokeRemote('diagnosis:fromJd', { campaignId: detailId });
      const msg = jobMessageFromEvents(events);
      await triggerSync();
      reload();
  ...
  if (detail) {
    return (
      <ScrollView style={{ flex: 1, backgroundColor: theme.bg }} contentContainerStyle={{ padding: 16, gap: 8 }}>
        ...
        <Pressable onPress={() => void diagnose()} style={{ backgroundColor: theme.accent, padding: 10, borderRadius: 8, alignItems: 'center' }}>
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.bg }} contentContainerStyle={{ padding: 16, gap: 10 }}>
      ...
      {campaigns.map((c) => (
        <Pressable key={c.id} onPress={() => setDetailId(c.id)} style={cardStyle}>
