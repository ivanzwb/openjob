...
| 源码 Agent 与 `file:line` 引用 | ✅ | ✅（包页面：读配对桌面的检出、问源码、存话术） |
...
- 业务表、`app_setting`（配置与密钥），以及岗位包声明的数据集合（承载表 `plugin_data`：仓库登记、产品案例、客户对话记录等）参与同步。
- 界面主题属于配置的一部分：默认浅色，在桌面端选定后随 `app_setting` 下发，手机端跟随，不单独设开关。已显式选过深色的配置不会被默认值覆盖。
- 岗位包的数据随同步到手机，包自带的页面（源码、案例训练、客户对话）在手机端也能打开；只有需要桌面原语的操作（仓库拉取、tree-sitter 索引）留在桌面完成。
- 搜索缓存、仓库本机路径（`local_path`）等设备本地属性各端独立，不同步。
