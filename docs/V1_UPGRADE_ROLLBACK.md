...
v1.0 引入三个岗位包（软件工程、产品经理、销售/客户成功），每个包内嵌自己的岗位能力
...
不需要回滚整个版本。能力的启用状态记在 Campaign 的运行描述符里，把对应
能力关掉即可：
...
| 验收项 | 守在哪 |
|---|---|
| 三个岗位包 golden 通过 | `plugins/{softwareEngineering,productManager,salesCustomerSuccess}/golden.test.ts` |
