...
> 状态：Draft（v2 重设计：两层模型 + 插入点，2026-09）  
...
手机端不重新解析依赖、不改变 Campaign 绑定；Campaign 固定版本的包数据拉取不到时，相关任务进入只读/不可用降级，不允许静默改用其他版本。不同步的内容：fixtures、测试，以及可执行代码——岗位包本来就不携带任何代码。手机端脱离桌面独立导入岗位包，需要端上验签基础设施，推迟到 Phase 4 生态化。
...
3. 第三阶段支持带签名和来源可信校验的第三方岗位包；
4. 在具备沙箱、权限提示、撤销和审计前，不加载任何可执行插件代码——岗位包永远是数据。
