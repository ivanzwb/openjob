...
题目与推荐答案本身也缓存在 `knowledge_node.quiz_question_md` / `quiz_recommended_answer_md` 上，按考点一份，随 `knowledge_node` 同步。再次打开考我直接显示保存的题，只有点「重新出题」才会换题并清空推荐答案。
...
界面上「显示出来的内容」都要能选中复制：模型生成的题目、答案、讲解、情报，以及报错信息——报错尤其要能复制，不然用户只能对着屏幕转述。

桌面端天生就是这样，不要加全局 `user-select: none` 把它关掉。手机端相反：React Native 的 `<Text>` 默认不可选中，得逐处写 `selectable`。**不能图省事在全局给所有 `Text` 打开**——`selectable` 会接管长按手势，按钮和标签上打开它，长按按钮会弹出文本选择菜单，把点击手势也拖慢。所以按内容分：模型输出、用户写的正文、报错文案加 `selectable`；按钮文字、状态标签、列表项里那些点了会跳转的标题不加。走 `MarkdownPreview` 渲染的内容已经统一处理过，不必再加。
